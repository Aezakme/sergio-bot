...
from src.utils.handlers_helpers import is_command_enabled_for_chat, check_command_is_off
...
re_gdeleha = re.compile(
    r"(где л[её]ха|л[её]ха где)[!?.]*\s*$", re.IGNORECASE | re.MULTILINE
)
...
@run_async
def send_slavic(chat_id, bot, update):
    texts = ["A мороженное какое было?", "Ммм коммунизм", "деньги - грязные бумажки"]
    bot.sendMessage(
        chat_id,
        random.choice(texts),
...
@run_async
def send_antifa(chat_id, bot, update):
    sticker_id = "CAACAgIAAxkBAAEFiJpmSfPg5lv_b4WT0xHcNp5-J2fC1wAChkUAAuTcUUrd-geBg8Yi3TUE"
    bot.sendSticker(chat_id, sticker_id, reply_to_message_id=update.message.message_id)

@run_async
def send_random_sticker_from_stickerset(
    bot: telegram.Bot, chat_id: int, stickerset_name: str
) -> None:
...
def message_reactions(bot: telegram.Bot, update: telegram.Update) -> None:
    ...
    if is_command_enabled_for_chat(chat_id, "suicide") and re_suicide.search(msg_lower):
        ...
        return
    if (
        is_command_enabled_for_chat(chat_id, CMDS["common"]["orzik"]["name"])
    ...
    if "слава" in words_lower or "славик" in words_lower:
        send_slavic(chat_id, bot, update)   
        
    if "хохол" in words_lower or "хохлы" in words_lower or "хохля" in words_lower:
        send_antifa(chat_id, bot, update)
...
@run_async
def call_osenya(
    bot: telegram.Bot, update: telegram.Update, key_media_group: str, img_url=None
):
    if img_url is None:
...
def random_khaleesi(bot, update):
    ...
    if not is_command_enabled_for_chat(chat_id, "random_khaleesi", True):
        return
    if not is_command_enabled_for_chat(
        chat_id, CMDS["common"]["khaleesi"]["name"]
    ) and not is_command_enabled_for_chat(chat_id, "random_khaleesi", False):
        return
    if RandomKhaleesi.is_its_time_for_khaleesi(
        chat_id
    ) and RandomKhaleesi.is_good_for_khaleesi(text):
